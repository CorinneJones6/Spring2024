...
class EncFS(Operations):
    ...
    def __init__(self, root):
        self.root = root
        self.openFiles = {} #Stores the root and the FD
        self.netFD = 3 
    ...
    def getattr(self, path, fh=None):
        ...
        # Check if the file's decrypted content is stored in memory.
        if full_path in self.inMemoryFiles:
             # If yes, update the file size to the size of the decrypted content.
            fileContent = self.inMemoryFiles[full_path]
            props['st_size'] = len(fileContent)
...
'''
    ...
    def decrypt(self, salt, encrypted_content):
        ...
        #password was input as a string and this needs bytes, need to convert to bytes 
    ...
    def write(self, path, buf, offset, fh):
        ...
        #TODO is this the correct error to send back?  
        if full_path not in self.openFiles:
            return -1 
    ...
    def truncate(self, path, length, fh=None):
        ...
        # Calculate the difference btwn the length and the current fileContent
        offset = length - len(fileContent)

        # If < 0, this means the provided length is smaller than the current length, and have slice to the length 
        if offset < 0: 
            fileContent = fileContent[:length] 
        # If > 0, this means the provided length is longer than the current length, and have to fill with zeros the difference (i.e., offset)
        elif offset > 0:
            fileContent += b'\x00' * (offset)

        #Store the updated content 
        self.inMemoryFiles[full_path] = fileContent
   ...
   '''         
    ...
    def release(self, path, fh):
        ...
        encrypted_data = self.encrypt(fileContent)

        # Reflect changes on the disk, 'wb' stands for writing in binary mode 
        with open(full_path, 'wb') as file:
            file.write(encrypted_data)
       
        # Clear out the file in virtual memory 
        self.inMemoryFiles.pop(full_path, None)
        self.openFiles.pop(full_path, None)
